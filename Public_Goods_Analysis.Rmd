...
ExploratoryEmmeans <- emmeans(ExploratoryAnalysis, ~ group.lockdown_cost)
```
...
predictionIncentivisedPlot <-
  ggplot(infectionData, aes(x = factor(group.total_infections), y = Mean, col = Type)) +
  geom_point() +
  geom_line() +
  ...
  scale_y_continuous(breaks = c(0, 1, 2, 3, 4), 
                     ...
                     limits = c(0,4)) +
  labs(x = "Number of positive cases", y = "Self-isolation level") +
...
h2bPlot <-
  ...
  scale_x_continuous(breaks = c(0, 1, 2, 3, 4), 
                     ...
                     minor_breaks = c(.5, 1.5, 2.5, 3.5),
                     limits = c(-0.2,4.2)) +
  labs(x = "Assumed self-isolation by others", y = "Hypothetical self-isolation") +
  theme_light()

ggarrange(h2aPlot, h2bPlot) +
  ggsave("H2Plots.png", height = 4, width = 9)
...
ggplot(lockdownCostData, aes(x = factor(group.lockdown_cost), y = SelfIsolation)) +
...
ggplot(lockdownCostData, aes(x = factor(group.lockdown_cost), y = player.contribution)) +
  geom_violin()

```

```{r}
# Hypotheticals by infection levels


HypotheticalInfectionData <-
  H2bData %>%
  filter(group.total_infections < 6) %>%
  group_by(AssumedOthersSelfIsolation, group.total_infections) %>%
  summarise(Hypothetical = mean(HypotheticalSelfIsolation, na.rm = T),
            ci = sd(HypotheticalSelfIsolation, na.rm = T)/sqrt(n())*1.96)

ggplot(HypotheticalInfectionData, aes(x = AssumedOthersSelfIsolation, y = Hypothetical, col = factor(group.total_infections))) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin = Hypothetical-ci, ymax = Hypothetical+ci), width = .2) + 
  scale_y_continuous(breaks = c(0, 1, 2, 3, 4), 
  ...
  scale_x_continuous(breaks = c(0, 1, 2, 3, 4), 
                     ...
                     minor_breaks = c(.5, 1.5, 2.5, 3.5),
                     limits = c(-0.2,4.2)) +
  scale_color_viridis_d(begin = .1, end = .85) +
  theme_light()
...
ggplot(lockdownCostAssumptionData, 
  ...
  scale_x_continuous(breaks = c(0, 1, 2, 3, 4), 
                     ...
                     minor_breaks = c(.5, 1.5, 2.5, 3.5),
                     limits = c(-.2,4.2)) +
  labs(x = "Others' assumed self-isolation", y = "Hypothetical self-isolation", col = "Cost of lockdown") +
  scale_colour_viridis_d(begin = .1, end = .8) +
  theme_light()
