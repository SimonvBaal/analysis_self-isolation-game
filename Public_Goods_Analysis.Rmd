...
```{r Datasets for each hypothesis}
...
ggplot(exp1GameData, aes(x = player.others_prediction, y = player.contribution)) +
...
ggplot(exp1GameData %>% 
         group_by(subsession.round_number, group.lockdown_cost) %>% 
         summarise(player.contribution = mean(player.contribution, na.rm = T)), 
       aes(x = subsession.round_number, y = player.contribution, col = factor(group.lockdown_cost))) + 
  geom_point() + 
  geom_smooth(method = 'lm') + 
  theme_light()
