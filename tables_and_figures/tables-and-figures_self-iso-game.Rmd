...
dataInfections <-
  ...
  summarise(
    ...
    Incentivised_Ci = sd(player.contribution, na.rm = T) / sqrt(n()) *
      1.96,
    Beliefs_Mean = mean(player.others_prediction, na.rm = T),
    Beliefs_Ci = sd(player.others_prediction, na.rm = T) / sqrt(n()) *
      1.96
  ) %>%
  pivot_longer(
    cols = Incentivised_Mean:Beliefs_Ci,
    names_to = c("Type", "Metric"),
    names_sep = "_",
    values_to = "Value"
  ) %>%
  pivot_wider(names_from = "Metric", values_from = "Value") %>%
  mutate(Type = factor(Type, levels = c("Incentivised", "Beliefs")))


plotBeliefsIncentivised <-
...
ggsave("plot_illusory-superiority.png", width = 7.5, height = 5.4)
...
plotBeliefs <- 
  ...
  scale_x_continuous(breaks = c(0, 10, 20, 30, 40), 
                     ...
                     minor_breaks = c(5, 15, 25, 35),
                     limits = c(-2, 42)) +
  labs(x = "Beliefs about others' behaviour", y = "Incentivised self-isolation") +
  theme_light()
...
plotLockdownCostHypothetical <-
  ...
  scale_x_continuous(breaks = c(0, 10, 20, 30, 40), 
                     ...
                     minor_breaks = c(5, 15, 25, 35),
                     limits = c(-2, 42)) +
  labs(x = "Self-isolation of others in scenario", 
